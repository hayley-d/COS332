...
pub mod question_api {
    ...
    /// # Returns
    /// A `Response` object with the appropriate content and status code.
    async fn handle_post(request: Request, state: Arc<Mutex<State>>) -> Response {
        let response = Response::default()
            .await
            .compression(request.is_compression_supported())
        ...
        if request.uri == "/answer" {
            ...
            if let Ok(payload) = payload {
                ...
                if let Some(uuid) = uuid {
                    let question = match state.lock().await.questions.get(&uuid) {
                        Some(q) => q,
                        None => {
                            return response
                                .body(String::from("Invalid question ID").into())
                                .code(HttpCode::BadRequest);
                        }
                    }
                    .clone();

                    if question.check_answer_correct(&payload.answers) {
                        state
                            .lock()
                            .await
                            .user_scores
                            .entry(client_id.to_string())
                            .and_modify(|(score, totoal)| {
                                *score += 1;
                                *totoal += 1;
                            })
                            .or_insert((1, 1));
